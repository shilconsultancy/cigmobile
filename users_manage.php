<?php
// users_manage.php (with Full CRUD and Special Owner Permissions)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if (isset($_POST['add_new_user'])) {
            $full_name = trim($_POST['full_name']); $username = trim($_POST['username']); $password = trim($_POST['password']); $role = trim($_POST['role']); $reports_to = (int)$_POST['reports_to']; $assigned_category_ids = $_POST['category_ids'] ?? [];
            ...
            if (empty($full_name) || empty($username) || empty($password) || empty($role) || empty($reports_to)) { throw new Exception('All fields except categories are required.'); }
        ...
        elseif (isset($_POST['update_user'])) {
            $user_id_to_update = (int)$_POST['user_id']; $full_name = trim($_POST['full_name']); $username = trim($_POST['username']); $password = trim($_POST['password']); $role = trim($_POST['role']); $reports_to = (int)$_POST['reports_to']; $assigned_category_ids = $_POST['category_ids'] ?? [];
            $manager_allowed_cat_ids = array_column($manager_accessible_categories, 'id');
            $is_permission_valid = empty(array_diff($assigned_category_ids, $manager_allowed_cat_ids));
            if (empty($full_name) || empty($username) || empty($role) || empty($reports_to)) { throw new Exception('Full name, username, role, and reports to are required.'); }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.body.addEventListener('click', function(e) {
        if (e.target.classList.contains('edit-user-btn')) {
            ...
            document.getElementById('edit-reports-to').value = btn.dataset.reportsTo;
