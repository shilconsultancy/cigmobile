<?php
// reports_team.php (Final Version with Context-Aware Back Button)
...
try {
    ...
    if ($view_user['role'] === 'sales') {
        ...
        $invoice_stmt = $pdo->prepare(
            "SELECT customer_name, DATE(order_date) as invoice_date, SUM(total_amount) as invoice_total,
             GROUP_CONCAT(CONCAT_WS('||', p.name, total_pcs, total_amount) SEPARATOR ';;') as product_details
             FROM orders o JOIN products p ON o.product_id = p.id
             WHERE user_id = ?
             GROUP BY customer_name, DATE(order_date) ORDER BY invoice_date DESC"
        );
...
<div class="bg-white p-4 sm:p-6 lg:p-8 rounded-xl shadow-lg max-w-7xl mx-auto space-y-6">
    ...
    <?php elseif ($display_mode === 'invoices'): ?>
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50"><tr><th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Invoice Date</th><th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Customer</th><th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">Invoice Total</th><th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase">Actions</th></tr></thead>
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php if (empty($page_data)): ?>
...
<div id="invoice-modal" class="hidden fixed inset-0 bg-gray-800 bg-opacity-75 z-50 flex items-center justify-center p-4">
    <div class="relative bg-white rounded-lg shadow-xl p-6 sm:p-8 w-full max-w-2xl mx-auto">
        ...
        <div class="text-sm text-gray-600 mb-4 border-b pb-2 space-y-1"><p><strong>Customer:</strong> <span id="modal-customer"></span></p><p><strong>Date:</strong> <span id="modal-date"></span></p></div>
        <div class="overflow-y-auto max-h-80"><table class="min-w-full divide-y divide-gray-200"><thead class="bg-gray-50"><tr><th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Product</th><th class="px-4 py-2 text-right text-xs font-medium text-gray-500 uppercase">Quantity (Packets)</th><th class="px-4 py-2 text-right text-xs font-medium text-gray-500 uppercase">Amount</th></tr></thead><tbody id="modal-products-body" class="bg-white divide-y divide-gray-200"></tbody></table></div>
        <div class="mt-4 pt-4 border-t flex justify-end items-center"><span class="text-lg font-bold text-gray-800">Total: <span id="modal-total" class="text-green-700"></span></span></div>
    </div>
</div>
<script>
